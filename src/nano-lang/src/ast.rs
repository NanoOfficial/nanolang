...
impl TypedModule {
    ...
    pub fn validate_module_name(&self) -> Result<(), Error> {
        if self.name == "nano" || self.name == "nano/builtin" {
...
pub enum Error {
    ...
    #[diagnostic(url("https://github.com/NanoOfficial/nanolang"))]
...
- nano: where the prelude is located;
- nano/builtin: where low-level Plutus builtins are stored

Note that 'nano' is also imported by default; but you can refer to it explicitly to disambiguate with a local value that would clash with one from that module."#
    ))]
