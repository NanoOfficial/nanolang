...
#[error(
    "Some of the labels used in this expression are unknown, highlighted them just below."
)]
...
impl Diagnostic for UnknownLabels {
    fn help<'a>(&'a self) -> Option<Box<dyn Display + 'a>> {
        Some(Box::new(formatdoc! {
            r#"Here's a list of all the (valid) labels:
...
#[derive(Debug, thiserror::Error, Diagnostic, Clone)]
pub enum Error {
    #[error("discovered a type cast from data without an annotation")]
    #[diagnostic(code("illegal::type_cast"))]
    #[diagnostic(help("Try adding up an annotation...\n\n{}", format_suggestion(value)))]
    CastDataNoAnn {
    ...
    #[error("struggling to unify the types of two expressions.\n")]
    #[diagnostic(url("https://github.com/NanoOfficial/nanolang#expressions"))]
    ...
    #[error("Got caught in an infinite cycle of type definitions.\n")]
    #[diagnostic(url("https://github.com/NanoOfficial/nanolang#type-aliases"))]
    ...
    #[error(
        "Found two function arguments both called '{}'.\n",
    ...
    #[error("Found duplication declarations for the constant '{}'.\n", name.purple())]
    ...
    #[error(
        "Stumbled upon the field '{}' twice in a data-type definition.\n",
        label.if_supports_color(Stdout, |s| s.purple())
    )]
...
"#
    ...
    #[error(
        "Noticed you were importing '{}' twice.\n",
...
Otherwise, just remove the redundant import."#
    ...
    #[error(
        "Discovered two top-level objects referred to as '{}'.\n",
        name.if_supports_color(Stdout, |s| s.purple())
    )]
...
"#,
    ...
    #[error(
        "Found two types declared with the same name: '{}'.\n",
        name.if_supports_color(Stdout, |s| s.purple())
    )]
    ...
    #[diagnostic(url(
        "https://github.com/NanoOfficial/nanolang#clause-patterns"
    ))]
    ...
    #[error(
        "Tried over an extra variable in an alternative pattern: {}.\n",
        name.if_supports_color(Stdout, |s| s.purple())
    )]
    #[diagnostic(url(
        "https://github.com/NanoOfficial/nanolang#alternative-clause-patterns"
    ))]
    ...
    #[error("Found a data type that has a function type in it. This is not allowed.\n")]
    ...
    #[error("Found a discarded expression not bound to a variable.\n")]
    ...
    #[error(
        "Found {} field{} in a context where there should be {}.\n",
        given.if_supports_color(Stdout, |s| s.purple()),
        if *given <= 1 { "" } else { "s"},
        expected.if_supports_color(Stdout, |s| s.purple()),
    )]
    #[diagnostic(url("https://github.com/NanoOfficial/nanolang#custom-types"))]
    ...
    #[error(
        "Found a function or constructor that expects {} arguments be called with {} arguments.\n",
        expected.if_supports_color(Stdout, |s| s.purple()),
        given.if_supports_color(Stdout, |s| s.purple())
    )]
    #[diagnostic(url("https://github.com/NanoOfficial/nanolang##named-functions"))]
...
Note that Nano supports argument capturing using '{discard}' as placeholder for arguments that aren't yet defined. This is like currying in some other languages.
...
"#
    ...
    #[error(
        "Found a pattern on a constructor that has {} field(s) be matched with {} argument(s).\n",
        expected.if_supports_color(Stdout, |s| s.purple()),
        given.len().if_supports_color(Stdout, |s| s.purple())
    )]
    #[diagnostic(url("https://github.com/NanoOfficial/nanolang#matching"))]
    ...
    #[error(
        "Found saw a pattern on a {}-tuple be matched into a {}-tuple.\n",
        expected.if_supports_color(Stdout, |s| s.purple()),
        given.if_supports_color(Stdout, |s| s.purple())
    )]
    #[diagnostic(url("https://github.com/NanoOfficial/nanolang#destructuring"))]
    ...
    #[error(
        "Found a generic data-type with {} type parameters instead of {}.\n",
        given.if_supports_color(Stdout, |s| s.purple()),
        expected.if_supports_color(Stdout, |s| s.purple()),
    )]
    #[diagnostic(url("https://github.com/NanoOfficial/nanolang#generics"))]
...
─▶  {suggestion}"#
    ...
    #[error(
      "Found the module '{}' contains the keyword '{}', which is forbidden.\n",
      name.if_supports_color(Stdout, |s| s.purple()),
      keyword.if_supports_color(Stdout, |s| s.purple()),
    )]
    #[diagnostic(url("https://github.com/NanoOfficial/nanolang#modules"))]
    ...
    #[error("Found a function which is ending with an assignment.\n")]
    #[diagnostic(url(""))]
    #[diagnostic(code("illegal::return"))]
    #[diagnostic(help(r#"In Nano, functions must return an explicit result in the form of an expression. While assignments are technically speaking expressions, they aren't allowed to be the last expression of a function because they convey a different meaning and this could be error-prone.
...
{sample}"#
    ...
    #[error(
        "found a missing variable in an alternative pattern: {}.\n",
        name.if_supports_color(Stdout, |s| s.purple())
    )]
    #[diagnostic(url(
        ""
    ...
    #[error("found a multi-validator where both take the same number of arguments.\n")]
    ...
    #[error(
        "stumbled upon an invalid (non-local) clause guard '{}'.\n",
        name.if_supports_color(Stdout, |s| s.purple())
    )]
    #[diagnostic(url(""))]
    ...
    #[diagnostic(help(
        r#"Because you used a tuple-index on an element, I assumed it had to be a tuple or some kind, but instead found:
...
To fix this, you'll need to either turn that argument as a labeled argument, or make the next one positional."#))]
    ...
    #[diagnostic(help(r#"found a public value that is making use of a private type. This would prevent other modules from actually using that value because they wouldn't know what this type refer to.
...
"#
    ...
    #[error(
        "stumbled upon a reference to an unknown module: '{}'\n",
        name.if_supports_color(Stdout, |s| s.purple())
    )]
    ...
    #[error(
        "found an unknown import '{}' from module '{}'.\n",
        name.if_supports_color(Stdout, |s| s.purple()),
    ...
    #[error("found a reference to an unknown type.\n")]
    ...
    #[error(
        "found a reference to an unknown data-type constructor: '{}'.\n",
        name.if_supports_color(Stdout, |s| s.purple())
    )]
    ...
    #[error("found a reference to an unknown variable.\n")]
...
The best thing to do from here is to remove it."#))]
    ...
    #[diagnostic(help(r#"While analyzing the return type of your validator, found it to be:
    ...
    match situation {
        ...
        None => formatdoc! {
            r#"I am inferring the following type:
               ...
               but found an expression with a different type:
...
fn suggest_make_public() -> String {
    formatdoc! {
        r#"Did you forget to make this value public?
           ...
           The function 'foo' is private and can't be accessed from outside of the 'nano/foo' module. But the data-type '{type_Bar}' is public and available.
        "#
        , keyword_fn = "fn".if_supports_color(Stdout, |s| s.yellow())
        , keyword_pub = "pub".if_supports_color(Stdout, |s| s.bright_blue())
...
fn suggest_import_constructor() -> String {
    formatdoc! {
        r#"Did you forget to import it?
           ...
           Data-type constructors are not automatically imported, even if their type is imported. So, if a module 'nano/pet' defines the following type:
...
pub enum Warning {
    #[error("found a record update using all fields; thus redundant.\n")]
    #[diagnostic(url(""))]
    ...
    #[error("found a record update with no fields; effectively updating nothing.\n")]
    #[diagnostic(url(""))]
    ...
    #[error("found a when expression with a single clause.")]
    #[diagnostic(
    ...
    #[error(
        "found an {} trying to match a type with one constructor",
    ...
    #[error("found a todo left in the code.\n")]
    ...
    #[error("found a type hole in an annotation.\n")]
    ...
    #[error(
        "found an unused private function: '{}'.\n",
        name.if_supports_color(Stderr, |s| s.purple()),
    )]
    #[diagnostic(help(
    ...
    #[error(
        "found an unused (private) module constant: '{}'.\n",
        name.if_supports_color(Stderr, |s| s.purple())
    )]
    #[diagnostic(help(
        "Perhaps your forgot to make it public using the '{keyword_pub}' keyword?\n\
